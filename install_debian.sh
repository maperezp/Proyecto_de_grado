#!/bin/bash -e
# PROTON-NG VERSION
# changes by: Diego Medina, diego.medina@procteck.com
...
PROTON_NG_DTB="imx8mp-var-som-proton-ala.dtb"
PROTON_UENV="uEnv.txt"
...
install_rootfs_to_emmc()
{
	...
	zstd -dc ${IMGS_PATH}/${ROOTFS_IMAGE} | tar xf - -C ${MOUNTDIR}

	# Change permissions to enable boot
	chown -R root:root ${MOUNTDIR}

	# Copy Proton NG dtb files
	cp  ${IMGS_PATH}/${PROTON_NG_DTB} ${MOUNTDIR}/boot/

	# Copy uEnv
	cp  ${IMGS_PATH}/${PROTON_UENV} ${MOUNTDIR}/boot/
...
{
	echo
	blue_bold_echo "Proton-ng's debian installed successfully"
	exit 0
}
...
blue_underlined_bold_echo "*** PROTON-NG, Variscite MX8 Debian eMMC Recovery ***"
echo
